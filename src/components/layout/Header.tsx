...
/**
 * Header – Top navigation bar. Standalone, no imports from old structure.
 */
export const Header: React.FC = () => {
  ...
  return (
    <header
      ...
      style={{ height: '64px', boxShadow: '0px 1px 4px rgba(0,0,0,0.06)' }}
    >
      ...
      <div className="flex items-center gap-2 ml-4">
        ...
        <div className="relative">
          ...
          >
            <Bell size={20} />
            <span className="absolute top-1.5 right-1.5 w-2 h-2 rounded-full border-2 border-white" style={{ backgroundColor: '#ef5350' }} />
          </button>

          {notifOpen && (
            <div className="absolute right-0 top-11 w-72 bg-white rounded-lg border border-gray-100 z-50 overflow-hidden"
        ...
        <button className="flex items-center justify-center rounded-full hover:opacity-80 transition-opacity ml-1">
          <div className="w-9 h-9 rounded-full flex items-center justify-center text-white text-xs font-bold" style={{ background: '#041627' }}>
            AD
          </div>
        </button>
      </div>
